...
fit_lm_all <- 
  dat_lfc %>%
    mutate(dat_lfc, Inhibition = 1 - Viability) %>% 
    select(-Treatment, -Akt1, -Cell, -Viability) %>% 
  lm(
  formula = Inhibition ~ 0 + ., 
  data = .) 
...
```{r fit_lm_erkakt}
fit_lm_erkakt <- lm(
  formula = Inhibition ~  0 + BioAkt + ERK1, 
  data = mutate(dat_lfc, Inhibition = 1 - Viability))
...
(Note that we only consider the range where the fitted viability < 1, there are one or two outlier datapoints for which this is not the case).

```{r res fit_lm_all, message = FALSE}
p_res_linfull <- 
  ggplot(
    filter(left_join(broom::augment(fit_lm_all), dat_lfc), .fitted < 1.),
    aes(x = 1-.fitted, y = .resid)) +
    geom_point(aes(color = Cell)) +
    geom_smooth(span = 1) +
    scale_color_manual(name = "", values = color_lst, labels = label_lst) +
    cowplot::theme_cowplot(font_family = "Helvetica") +
    ...
    labs(titles = "Linear model", subtitle = "All epitopes", 
         x = "Fitted viability", y = "Residuals")

p_res_lin <-
  ggplot(
    filter(left_join(broom::augment(fit_lm_erkakt), dat_lfc), .fitted < 1.),
    aes(x = 1-.fitted, y = .resid)) +
    geom_point(aes(color = Cell)) +
    geom_smooth(span = 1) +
    scale_color_manual(name = "", values = color_lst, labels = label_lst) +
...
The results depend a bit on which metric is chosen, but overall the michaelis-menten like functions performs best and has the most straightforward interpretation, so we will continue with that.
...
tmp <- bind_rows(
  ...
  #filter(.fitted < 1.) %>% 
  select(Cell, Viability, .fitted, .resid)
...
tmp2 <- select(prediction_combined, Cell, Viability, .fitted = PredictedViability, .resid)
...
ggplot(prediction_combined,
  aes(x = Viability, y = PredictedViability)) +
  geom_abline(slope = 1, color = "lightgray") +
  geom_point(aes(color = Cell), size=1, shape=0) +
  labs(
    # title = "Correlation between measured and\nfitted viabilty",
    # subtitle = subtitle_mm,
    x = "Measured viability (Relative to DMSO)", y = "Viability fitted from ERK and AKT"
  ) +
  scale_x_continuous(expand = c(0, 0), limits = c(0,1.15)) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 1.15))+
  # #xlim(c(0, 1.15)) + ylim(c(0, 1.5)) + 
  # ggpubr::stat_cor(method = "pearson") +
  scale_color_manual(name = "", values = color_lst, labels = label_lst) +
...
ggsave(here('figures', 'correlations-and-predictions', 'viability-signaling-MM-all.pdf'),
        width=60/inch2mm, height = 60/inch2mm)
```
