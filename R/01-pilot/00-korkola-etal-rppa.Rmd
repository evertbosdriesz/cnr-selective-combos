...
We also plot the dymanics of AKT only.

```{r}
ggplot(
  filter(df_pao, epitope == "AKT_pS473(V)"),
  aes(x = time, y = expression, color = cell_line)) +
  geom_line() +
  geom_point() +
  facet_wrap(~treatment, scales = "free_y", nrow=1) +
  xlim(c(0, 8)) +
  labs(x = "Time (hrs)", y = "Normalized, log2-\ntransformed expression",
       title = "phospho-AKT dynamics in Korkola et al. ") +
  cowplot::theme_cowplot(font_family = "Helvetica", font_size = 6) +
  theme(legend.position = "bottom", legend.title = element_blank()) +
  guides(color=guide_legend(nrow=2,byrow=TRUE))


ggsave(here("figures", "Misc", "Korkula-time-course.pdf"),  height = 1.5, width = 6)
```
```
