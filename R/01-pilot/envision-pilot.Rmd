...
# Plot single curve
ggplot(
    filter(viabilities_filtered, Drug == "mTORi (AZD8055)"), 
    aes(x = Concentration, y = NormalizedViability, 
        color = Cell, group = Cell
    )) +
    # geom_vline(
    #     aes(xintercept = IC50, color = Cell),
    #     data = dat_ic50, show.legend = FALSE
    # ) +
    geom_smooth(show.legend = FALSE) +
    geom_point(shape = 1) +
    scale_x_log10(
    ...
    labs(x = "Drug concentration [µM]", y = "Viability (Relative to DMSO)", title = "mTORi (AZD8055)") +
    scale_color_manual(name = "", values = color_lst, labels = label_lst)
```
